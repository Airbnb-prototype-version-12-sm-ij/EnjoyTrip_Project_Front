...
const show = (mapRef) => {
  ...
  // 키워드로 장소를 검색합니다
  ps.keywordSearch(regionData[props.sidoCode]["name"] + " " + regionData[props.sidoCode]["children"][props.gugunCode]["name"] + " " + mapRef, placesSearchCB);
};
...
<template>
  <div class='flex-row mt-5'>
    ...
    <div class="flex space-x-2">
      <button @click.prevent="show('전기차충전소')" :class="{ 'border-black': type === '전기차충전소' }"
      ...
      <button @click.prevent="show('주유소')" :class="{ 'border-black': type === '주유소' }"
        class="inline-flex items-center justify-center whitespace-nowrap rounded-3xl text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 border-input bg-background hover:bg-accent hover:text-accent-foreground hover:bg-gray-200 h-10 px-4 py-2 border-2">
        주유소
      </button>
      <button @click.prevent="show('주차장')" :class="{ 'border-black': type === '주차장' }"
        class="inline-flex items-center justify-center whitespace-nowrap rounded-3xl text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 border-input bg-background hover:bg-accent hover:text-accent-foreground hover:bg-gray-200 h-10 px-4 py-2 border-2">
        주차장
      </button>
      <button @click.prevent="show('맛집')" :class="{ 'border-black': type === '맛집' }"
      ...
      <button @click.prevent="show('병원')" :class="{ 'border-black': type === '병원' }"
        class="inline-flex items-center justify-center whitespace-nowrap rounded-3xl text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 border-input bg-background hover:bg-accent hover:text-accent-foreground hover:bg-gray-200 h-10 px-4 py-2 border-2">
        병원
      </button>
    </div>
    {{ regionData[props.sidoCode]["name"] + " " + regionData[props.sidoCode]["children"][props.gugunCode]["name"] + " "
      + type }}
