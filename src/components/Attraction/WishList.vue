...
const getTripRoute = async () => {
  ...
  wishList.value = eval(response.choices[0].message.content.replace(/`/g, '').replace("javascript", ''));
...
const getExplanation = async () => {
  ...
  const response = await openai.chat.completions.create({
    messages: [
      ...
      {
        role: "user",
        content: "해당 여행 경로에 대한 설명, 여행 경로 순서를 정한 이유, 예상 소요시간, 경비등의 이유를 문단 사이에 띄어쓰기를 하여 마크다운 형식으로 분류별로 사용자 친화적으로 제공하시오  그리고 그 외의 답변은 허용하지 않습니다",
      },
    ],
    model: "gpt-3.5-turbo",
    max_tokens: 2000,
