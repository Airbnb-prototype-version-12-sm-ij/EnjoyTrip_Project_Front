...
<template>
  <div class="flex">
    ...
    <!-- 경로 및 chatgpt -->
    <div class="flex-col">
      <KakaoMobility :wishList="wishList" v-if="isInit" class='rounded-xl' />
      <div class="border flex items-start justify-start h-[815px] w-full max-w-[640px] rounded-xl">
        <div class="flex flex-col w-full">


          <div @click="getTripRoute"
            :style="{ width: '300px', backgroundColor: isLoading ? '#D3D3D3' : 'white', pointerEvents: isLoading ? 'none' : 'auto' }"
          ...
          <p v-html="markdown.render(result.content)" style="color: black" class="mx-14 mt-14" v-show="!isLoading"></p>
        </div>
      </div>
    </div>
  </div>
